...
export function initCarte(){
  ...
  // Groupe "Satellite + noms" = satellite + labels superposés
  const esriHybrid = L.layerGroup([esriSat, esriLabels])

  // OSM actif par défaut
  osmLayer.addTo(map)

  // Contrôle de sélection de fond (haut droite)
  L.control.layers({
    "🗺️ Carte OSM"        : osmLayer,
    "🛰️ Satellite"        : esriSat,
    "🛰️ Satellite + noms" : esriHybrid
  }).addTo(map)
  ...
  /* déplacement marker → recalcul route et météo
     keepView=true : on garde le zoom/centre actuel après un drag */
  marker.on("dragend", () => {
    const pos = marker.getLatLng()
    calculRoute([pos.lat, pos.lng], true)
...
══════════════════════════════════════ */
function calculRoute(dest, keepView = false){
  ...
  .then(data => {
    ...
    if(!keepView) map.fitBounds(routeLine.getBounds())
  })
}
